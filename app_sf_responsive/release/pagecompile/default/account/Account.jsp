...
new TagParameter("trailtext",localizeText(context.getFormattedValue("account.overview.link",null)))}, 5); %></div>
...
<h1><% _boolean_result=false;try {_boolean_result=((Boolean)(((((Boolean) (disableErrorMessages().isDefined(getObject("CurrentUser:Profile:DataSheet:FirstName")))).booleanValue() && ((Boolean) (((!(context.getFormattedValue(getObject("CurrentUser:Profile:DataSheet:FirstName"),null).equals(context.getFormattedValue("",null)))) ? Boolean.TRUE : Boolean.FALSE))).booleanValue()) ? Boolean.TRUE : Boolean.FALSE))).booleanValue();} catch (Exception e) {Logger.debug(this,"Boolean expression in line {} could not be evaluated. False returned. Consider using the 'isDefined' ISML function.",25,e);}if (_boolean_result) { %><% {out.write(localizeISText("account.overview.personal_message.text",null,null,encodeString(context.getFormattedValue(getObject("CurrentUser:Profile:DataSheet:FirstName"),null)),null,null,null,null,null,null,null,null,null));} %><% } else { %><% {out.write(localizeISText("account.overview.personal_message.text",null,null,encodeString(context.getFormattedValue(getObject("CurrentUser:Profile:Credentials:Login"),null)),null,null,null,null,null,null,null,null,null));} %><% } %></h1>
...
<h2><% {out.write(localizeISText("account.order.most_recent.heading","",null,null,null,null,null,null,null,null,null,null,null));} %></h2><% {out.flush();processLocalIncludeByServer((com.intershop.beehive.core.capi.request.ServletResponse)response,"account/overview/LatestOrders", null, "104");} %><h2><% {out.write(localizeISText("account.overview.wishlist.most_recent.heading","",null,null,null,null,null,null,null,null,null,null,null));} %></h2><% {out.flush();processLocalIncludeByServer((com.intershop.beehive.core.capi.request.ServletResponse)response,"account/overview/LatestWishlistItems", null, "108");} %><% {out.flush();processLocalIncludeByServer((com.intershop.beehive.core.capi.request.ServletResponse)response,"account/inc/PrivacyPolicyNote", null, "111");} %></div>
</div>
</div><% printFooter(out); %>