...
%><%
    ...
    if (pageletentrypoint != null && repository!= null)
    {
		...
		"CASE WHEN this.validfrom is null then 2 else 1 END,"+
        "this.validfrom ASC"
        ,args);
        
        Iterator<?> validFromDate_object=objs.iterator();
        if(validFromDate_object.hasNext())
        {
        	PageletPageletEntryPointAssignment assignment=po.cast(validFromDate_object.next());
        	getPipelineDictionary().put("MinPublishedFromDate", assignment.getValidFrom());
        	ORMHelper.closeIterator(validFromDate_object);
        }
    }
%><% printFooter(out); %>