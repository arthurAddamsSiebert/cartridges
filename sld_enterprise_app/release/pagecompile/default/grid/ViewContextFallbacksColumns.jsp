...
type: 'link',
dataKey: 'name',
title: '<%=context.getFormattedValue(localizeText(context.getFormattedValue("ViewContextTabDetails.Name.table_header",null)),null)%>',
url: '<% {String value = null;try{value=context.getFormattedValue(context.getFormattedValue(url(true,(new URLPipelineAction(context.getFormattedValue("ViewPageletEntryPoint-View",null)))),null) + context.getFormattedValue("?ContentRepositoryUUID=",null) + context.getFormattedValue(getObject("params:ContentRepositoryUUID"),null) + context.getFormattedValue("&LocaleId=",null) + context.getFormattedValue(getObject("params:Locale:LocaleID"),null) + context.getFormattedValue("&PageletEntryPointUUID={pageletentrypointid}&Page={page}",null) + context.getFormattedValue("&StartFlag=true",null),null,null);}catch(Exception e){value=null;Logger.error(this,"ISPRINT has an invalid expression. Returning empty string. Line: {16}",e);}if (value==null) value="";value = encodeString(value);out.write(value);} %>',
width: 265,
maxWidth: 265,
sortable: false
},
...
type: 'text',
dataKey: 'description',
title: '<%=context.getFormattedValue(localizeText(context.getFormattedValue("ViewContextTabDetails.Description.table_header",null)),null)%>',
sortable: false
},
type: {
type: 'text',
dataKey: 'type',
title: '<%=context.getFormattedValue(localizeText(context.getFormattedValue("ViewContextTabDetails.Type.table_header",null)),null)%>',
width: 265,
maxWidth: 265,
sortable: false
}
},
names = [<% {String value = null;try{value=context.getFormattedValue(getObject("columns"),null,null);}catch(Exception e){value=null;Logger.error(this,"ISPRINT has an invalid expression. Returning empty string. Line: {36}",e);}if (value==null) value="";out.write(value);} %>],
