...
function KupuRemoveElementButton(buttonid, element_name, cssclass) {
    ...
    this.button = window.document.getElementById(buttonid);
    this.onclass = cssclass;//'invisible';
    ...
    */
    ...
    this.setTextStyle = function(style) {
        ...
        if (this.editor.getBrowserName() == "IE") {
            ...
            if (selNode.id=="kupu-editable")
            {
            	var newNode = this.editor.getInnerDocument().createElement(style);
            	var text = serialize(this.editor.getSelectedNode());
	            newNode.innerHTML = text;
	            selNode.innerHTML ="";
	            selNode.appendChild(newNode);
	            return;
            }	
            else if (selNode.parentElement != null &&
              		selNode.parentElement.id=="kupu-editable")
            {
            	var newNode = this.editor.getInnerDocument().createElement(style);
            	var text = serialize(this.editor.getSelectedNode());
	            newNode.innerHTML = text;
				var parent = selNode.parentElement;
				parent.insertBefore(newNode, selNode);
...
function ColorchooserTool(fgcolorbuttonid, hlcolorbuttonid, colorchooserid) {
	...
	if (this.fgcolorbutton != null){
            this.fgcolorbutton.disabled = disabled;
	};
	if (this.hlcolorbutton != null){
            this.hlcolorbutton.disabled = disabled;
	};
    };
...
function LinkTool() {
    ...
    this.updateLink = function (linkel, url, type, name, target, title) {
        if (type && type == 'anchor') {
            ...
            if (serialize(linkel) == "") {
                var doc = this.editor.getInnerDocument();
                linkel.appendChild(doc.createTextNode(name));
			}            
        } else {
            linkel.href = url;
            if (serialize(linkel) == "") {
...
function ContextChooserTool ()
{
	this.currentContextTemplateName = 'htmleditor/TextOut';
	this.currentContextTemplate = '';
	this.xmhlttp=null;
	this.getCurrentContextTemplateName = function () {
		return this.currentContextTemplateName;
	}
	
	this.setCurrentContextTemplateName = function (template) {
		this.currentContextTemplateName = template;
	}

	this.setContextTemplateOrganization = function (organization) {
		this.contextTemplateOrganization = organization;
	}

	
	this.getCurrentContextTemplateCallback = function(innerCallback)
	{
   		if (this.xmlhttp.readyState == 4)
		{
			this.currentContextTemplate = this.xmlhttp.responseText;
			innerCallback.execute();
		}
	}
	this.getCurrentContextTemplate = function (innerCallback)
	{
    	this.xmlhttp = Sarissa.getXmlHttpRequest();
		this.xmlhttp.open('GET', 'HtmlEditor-GetTemplate?'+
					'TemplateName='+this.currentContextTemplateName+
					'&OrganizationDomainId='+this.editor.organization , true);
		var wrappedCallback = new ContextFixer(this.getCurrentContextTemplateCallback, this, innerCallback);
        this.xmlhttp.onreadystatechange = wrappedCallback.execute;
    	this.xmlhttp.send(null);
	}
}
ContextChooserTool.prototype= new KupuTool;
/**
 * KupuTool implementation that hndles the setting and previewing of templates
 **/
function TemplateTool (contextTool) {
	
	this.contextTool = contextTool;

	this.renderPreview = function(previewDocument, template) {
		var wrappedCallback = new ContextFixer(this._getTemplateCallback, this, previewDocument, template);
		this.contextTool.getCurrentContextTemplate(wrappedCallback);
	}

	this.setTemplate = function (template){
		this.editor.getInnerDocument().getElementsByTagName('body')[0].innerHTML = template;
	}   
	
	this._showPreviewCallback = function (previewDocument, contextTemplate) {
   		if (this.xmlhttp.readyState == 4) {
   		    
			previewDocument.getElementsByTagName('body')[0].innerHTML = 
				contextTemplate;
			
			previewDocument.getElementById('kupu-editable').innerHTML =	
				this.xmlhttp.responseText;
		}
    }

    this._getTemplateCallback = function(previewDocument, template) {
    	this.xmlhttp = Sarissa.getXmlHttpRequest();
		this.xmlhttp.open('GET', 'HtmlEditor-GetTemplate?TemplateName='+template
		+'&OrganizationDomainId='+this.editor.organization, true);
		var wrappedCallback = new ContextFixer(this._showPreviewCallback, this, previewDocument, this.contextTool.currentContextTemplate);
        this.xmlhttp.onreadystatechange = wrappedCallback.execute;
    	this.xmlhttp.send(null);
    }

}
TemplateTool.prototype= new KupuTool;


/**
 * KupuTool implementation that handles the setting and linking to anchors
 **/
function AnchorTool () {
}
AnchorTool.prototype= new KupuTool;

/**
 * KupuTool implementation that handles the setting and linking to anchors
 **/
function ExternalLinkTool () {
}
ExternalLinkTool.prototype= new KupuTool;
...
function TableTool() {
    ...
    this.createContextMenuElements = function(selNode, event) {
        ...
        if (!table) {
            ...
            var ret = new Array();
	    ret.push(new ContextMenuElement('Add table', this.addPlainTable, this));
...
function ListTool(addulbuttonid, addolbuttonid, ulstyleselectid, olstyleselectid) {
    ...
    this.setDisabled = function (disabled){    
        this.addulbutton.disabled = disabled;
        this.addolbutton.disabled = disabled;
...
function ViewSourceTool() {
    ...
    this.viewSource = function() {
        ...
        var contents = '<html>\n' + serialize(this.editor.getInnerDocument().documentElement) + '\n</html>';
...
function DefinitionListTool(dlbuttonid) {
    ...
    */
    ...
    this.setDisabled = function (disabled){
    	this.dlbutton.disabled = disabled;
    }
    this.initialize = function(editor) {
        /* initialize the tool */
        this.editor = editor;
