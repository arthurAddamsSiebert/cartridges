...
function TemplateDrawer(elementid, tool)
{
   /* Template drawer */
    this.element = document.getElementById(elementid);
    this.preview = document.getElementById('kupu-templates-preview');
    this.tool = tool;
    ...
    this.selectTemplate = function (id) {
    	...
    	{
    		this.selectedTemplate.className = "kupu-template";
    	}
    	newSelection.className = "kupu-template-selected";
    	this.selectedTemplate = newSelection;
    	this._showPreview(newSelection.id);
    }
    
    this._showPreview = function (template) {
    	tool.renderPreview(this.preview.contentWindow.document, template);
    }

	this.setTemplate = function (){
		if (this.selectedTemplate != null)
		{
			this.tool.setTemplate(serialize(this.preview.contentWindow.document.
				getElementsByTagName('body')[0]));
			drawertool.closeDrawer();
		}
	}   
}
TemplateDrawer.prototype = new Drawer;


function ExternalLinkDrawer (elementid, tool){
    this.element = document.getElementById(elementid);
    this.tool = tool;
    this.createExternalLink = function()
    {
    	var url = document.getElementById('kupu-externallink-url').value;
    	var target = document.getElementById('kupu-externallink-target').value;
    	var title = document.getElementById('kupu-externallink-title').value;
    	var ssl = document.getElementById('kupu-externallink-ssl').checked;
    	if (url.indexOf("pipeline://") != -1)
    	{
    		var params = "";
    		// remove protocol
    		url=url.substring("pipeline://".length);
    		// get site
    		var i = url.indexOf('/');
    		var site = url.substring(0,i);
    		if (site == ""){
    			alert("Please provide a valid site.");
    			return;
    		}
    		url = url.substring(i+1);
    		if (url == ""){
    			alert("Please provide a valid pipeline.");
    			return;
    		}
    		...
    		i = url.indexOf('?');
    		if (i != -1) {
    			var pipeline = url.substring(0,i);
    			url=url.substring(i+1);
    		...
    		url = "https://./?[islink]"+pipeline+"|"+site+"|";
    		if (params != "")
    			url+=params+"|";
    		if (ssl)
    			url += "kupussl|true|";
   		...
   		this.tool.createLink(url, "href", name, target, title);
		this.hide();    		
	}    
    
}
ExternalLinkDrawer.prototype = new Drawer;

function LinkToAnchorDrawer(elementid, tool)
{
   /* Template drawer */
    this.element = document.getElementById(elementid);
    this.tool = tool;
    ...
    this.createContent = function() {
	    this.selectedAnchor = null;
    	var listDiv = document.getElementById('linktoanchor-resource-items');
    	listDiv.innerHTML ='';
    	var anchors = this.editor.getInnerDocument().anchors;
    	
    	for (var i = 0; i< anchors.length; i++)
    	{
    		var newDiv = document.createElement('div');
    		newDiv.className= 'kupu-anchor-item';
    		newDiv.id = 'anchor'+i;
    		var callHelper = new ContextFixer(this.selectAnchor, this, newDiv.id);
    		newDiv.onclick = callHelper.execute;
    		var text = document.createTextNode(anchors.item(i).name);
    ...
    this.selectAnchor = function (id) {
    	...
    	{
    		this.selectedAnchor.className = "kupu-anchor-item";
    	}
    	newSelection.className = "kupu-anchor-item-selected";
    	this.selectedAnchor = newSelection;
    }
    this.createLinkToAnchor = function()
    {
    	if (this.selectedAnchor != null)
    	{
	    	// link needs to be encoded becaus IE will prefix a #foo href
	    	// with the docroot of the editor
	    	// provide a title for links with no selected text
	    	tool.createLink(
	    		'https://./?[islink]#'+this.selectedAnchor.innerText+"|[/islink]", 
	    		'', 
	    		/*name*/  '', 
	    		/*target*/ null, 
	    		/*title*/ this.selectedAnchor.innerText);
	    }
	    else
	    {
...
LinkToAnchorDrawer.prototype = new Drawer;


function SetAnchorDrawer(elementid, tool)
{
   /* Anchor drawer */
    this.element = document.getElementById(elementid);
    this.tool = tool;
    ...
    {
    	var name = document.getElementById('kupu-setanchor-name').value;
    	var title = document.getElementById('kupu-setanchor-title').value;
    	if (name != null && name != '')
    	{
	    	tool.createLink(
	    		'.', 
	    		...
	    		/*target*/ null, 
	    		/*title*/ title);
	    	// reset html of editor to ensure the new anchors appear in the dom	
	    	var bodyHTML = this.editor.getHTMLBody();
	    	this.editor.setHTMLBody(bodyHTML);
		}
	    else
	    {
	    	this.editor.updateState();
...
SetAnchorDrawer.prototype = new Drawer;
...
function LibraryDrawer(tool, xsluri, libsuri, searchuri) {
    ...
    */
    ...
    this._libsXslCallback = function(dom) {
        ...
        */
        ...
        try {
            ...
            this.xsltproc.setParameter("", "scriptBase", scriptBase);
	...
	this._createPathLink=function(node)
	{
		var link = document.createElement("a");
		var callback = null;
		if (node.nodeName == "collection")
		{
			callback = new ContextFixer(this.selectCollection,this,
				node.getAttribute('id'));
		}	
		else
		{
			callback = new ContextFixer(this.selectLibrary,this,
				node.getAttribute('id'));
		}
		link.onclick=callback.execute;
		link.innerHTML = node.selectSingleNode("title/text()").nodeValue+"["+ node.getAttribute('id')+"]";
		return link;
	}
    ...
    this.updateDisplay = function(id) {
       ...
       */
		...
		var sourcenode = this.xmldata.selectSingleNode('//*[@selected]');
		/*
		if (sourcenode != null)
		{
			var pathdiv = document.getElementById("drawer-path");
			pathdiv.innerHTML="";
			var link = this._createPathLink(sourcenode);
			
			var textnode = document.createTextNode(" > ");
			pathdiv.appendChild(textnode);
			pathdiv.appendChild(link);
			
			if (sourcenode.nodeName == "collection")
			{
				var id = sourcenode.getAttribute('id');
				var xpath = "//collection[@loadedInNode='"+id+"']";
				var parent = this.xmldata.selectSingleNode(xpath);
				if (parent != null)
				do
				{
					...
					link = this._createPathLink(parent);
					pathdiv.insertBefore(link,textnode);
					
					var newtextnode = document.createTextNode(" > ");
					pathdiv.insertBefore(newtextnode,link);
					textnode = newtextnode; 
					id = parent.getAttribute('id');
					xpath = "//collection[@loadedInNode='"+id+"']";
					parent = this.xmldata.selectSingleNode(xpath);
				} while (parent != null)
			}
			
			//alert(sourcenode.nodeName+" "+sourcenode.getAttribute('id'));
			document.getElementById(sourcenode.getAttribute('id')).className=
				'kupu-libsource-selected';
		}*/
    };
...
function ImageLibraryDrawer(tool, xsluri, libsuri, searchuri) {
    ...
    this.save = function() {
		...
		//changed!	
        var uri = selnode.selectSingleNode('preview/text()').nodeValue;
        
		var encodedUriNode = selnode.selectSingleNode('encodeduri/text()');
		var encodedUri = encodedUriNode!= null ? encodedUriNode.nodeValue:null; 
		...
		{
			img.setAttribute('encodeduri', encodedUri);
		}

        // Set image class from the alignment radio buttons
        var radios = document.getElementById('image-align');
        var align = radios.options[radios.selectedIndex].value;
...
function LinkLibraryDrawer(tool, xsluri, libsuri, searchuri) {
    ...
    this.save = function() {
        ...
        var ssl = document.getElementById('link_ssl').checked;
        if (ssl){
        	uri = uri.substring(0,uri.length-9);	
        	uri += "kupussl|true|[/islink]";
        }
        
        var name = '';
        name = selnode.selectSingleNode('title/text()').nodeValue;
        name = name.strip();
