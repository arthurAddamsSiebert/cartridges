...
var _getValueMap= {
	"COORDS" : function(node, attribute) {
	...
	"STYLE" : function(node, attribute){
		return attribute.specified ? 
			(_SARISSA_IS_IE ? node.style.cssText : attribute.nodeValue) :
			null;
	},
	"SHAPE" : function (node, attribute) {
		return  attribute.nodeValue != "" ?
			attribute.nodeValue :
			null;
	}
...
function _getAttributeValue(node, attribute){
	var getValue = _getValueMap[attribute.nodeName.toUpperCase()];
	if (getValue != null) 
		return getValue(node, attribute);
	else
		return  attribute.nodeValue != null && attribute.nodeValue != "" && attribute.specified?
			attribute.nodeValue :
			null;
}

function _serialize(node, omitOuter){
	var output="";
	...
	if (node.nodeType != 3){
		...
		if (!omitOuter){
			...
			if (attributes != null && attributes.length > 0)
				for(var i = 0; i < attributes.length; i++){
					...
					var value = _getAttributeValue(node, attribute);
					if (value != null)
		...
		if (node.nodeName.toUpperCase()=="SCRIPT") {
			output += node.innerHTML + "</"+node.nodeName+">";
			return output;
		}	
		if (kids != null && kids.length > 0)
			for (var i = 0; i < kids.length ; i++)
				output += _serialize(kids[i],false);
		// haque for ie				
		else if (node.nodeName.toUpperCase()=="P")
			output += "&nbsp;"
					
		if (!omitOuter && (kids != null && kids.length > 0) && (node.nodeName != "br"))		
			output += "</"+node.nodeName+">";
	} else {
...
function serialize(node){ 
	return _serialize(node, true);
}
function _walk (node, condition, action) {
	var kids = node.childNodes;
	if (kids != null && kids.length > 0)
		for (var i = 0; i < kids.length ; i++)
			_walk(kids[i], condition, action); 
	if (condition(node))
		action(node);
}

function MediaObjectFilter() {
	...
	this.filter = function(ownerdoc, htmlnode){
		_walk(htmlnode, 
			...
			/* action */
			function(node){
				var encodedUri = node.getAttribute('encodeduri');
				if (encodedUri != null) {
					node.removeAttribute('encodeduri');
					node.setAttribute('src', encodedUri);
...
function ReverseMediaObjectFilter() {
	this.initialize = function(editor) {
		this.editor = editor;
	};
	this.filter = function(ownerdoc, htmlnode){
		_walk(htmlnode,
			...
			function(node) {
				...
				if (src.indexOf("[ismediaobject]")!= -1) {
					node.setAttribute('encodeduri', src);
					var start = src.lastIndexOf("|");        		
					var end = src.indexOf("[/ismediaobject]");      		
...
function XhtmlValidation(editor) {
    ...
    this.elements = new function(validation) {
        ...
        this.attributes = [
            ...
            'xml:lang','xml:space','xmlns', 'encodeduri', 'contentEditable'];
    ...
    this.tagAttributes = new function(el, validation) {
        ...
        this.body = ['background','bgcolor','text','link','vlink','alink'].concat(el.attrs, el.loadevents);		this.div = el.attrs.concat('contentEditable');
        ...
        this.img = el.attrs.concat('src','alt','name','longdesc','height','width','usemap','ismap','align','border','hspace','vspace', 'encodeduri');
    ...
    this._convertNodes = function(ownerdoc, htmlnode, xhtmlparent, permitted) {
        ...
        var nodename = this._getTagName(htmlnode);                var nostructure = !this.filterstructure;
