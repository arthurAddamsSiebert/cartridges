...
var defaultDataParser = function(data) {
	if ( typeof data == 'string' ) {
		...
		this._messages($.ui.assignment.constante.MESSAGE_ERROR, $.ui.assignment.locale.errorDataFormat);
		data = false;
	}
	return data;
};
...
$.widget("ui.assignment", {
  options: {
	...
	_create: function() {
		this.originalContent = this.element.children().clone();
		
		this.element.hide();
		this.busy = false;  // busy state
		this.container = $('<div class="ui-assignment ui-helper-clearfix ui-widget"></div>').insertAfter(this.element);
		...
		this.availableContainer.append('<div class="container-name">' + this.options.availableLabel + '</div>');
		this.selectedContainer.append('<div class="container-name">' + this.options.selectedLabel + '</div>');
		//this.selectedActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="count">'+$.tmpl($.ui.assignment.locale.itemsCount,{count:0})+'</span><a href="#" class="remove-all">'+$.tmpl($.ui.assignment.locale.removeAll)+'</a></div>').appendTo(this.selectedContainer);
		this.selectedActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="defaultvalue">'+$.tmpl($.ui.assignment.locale.defaultValue)+'</span><a href="#" class="remove-all">'+$.tmpl($.ui.assignment.locale.removeAll)+'</a></div>').appendTo(this.selectedContainer);
		this.availableActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="busy">'+$.tmpl($.ui.assignment.locale.busy)+'</span><input type="text" class="search ui-widget-content ui-corner-all"/><a href="#" class="add-all">'+$.tmpl($.ui.assignment.locale.addAll)+'</a></div>').appendTo(this.availableContainer);
		...
		this.notificationBox = $('<div class="ui-assignment-notification"></div>').hide().appendTo(this.selectedContainer);

		var that = this;

		// remove notification when a radiobutton is selected
		this.selectedContainer.delegate('input:radio', 'click', function () {
			that._notification();
		});

		// initialize data cache
		this.availableList.data('assignment.cache', {});
		this.selectedList.data('assignment.cache', {});
		...
		// batch actions
		this.container.find(".remove-all").bind('click.assignment', function() { that.selectNone(); return false; });
		this.container.find(".add-all").bind('click.assignment', function() { that.selectAll(); return false; });
    ...
    **************************************/
	...
	resize: function() {
		// set dimensions
		this.container.width(this.element.width()+ 20);
		...
		if (this.notificationBox.text() !== '') {
			notificationHeight = this.notificationBox.height() || 27;
		}
		this.selectedList.height(Math.max(this.element.height()-this.selectedActions.height()-notificationHeight,1));
	...
	selectedDefaultValue: function() {
		return this.selectedList.find('input:radio:checked').val() || '';
	},
	...
	search: function(query) {
		if (!this.busy && this.enabled() && this.options.searchable) {
			...
			input.val(query);
			input.trigger('keydown.assignment');
		}
	},
    ...
    **************************************/
	...
	_notification: function(msg) {
		var notificationHeight = 27;
		
		
		if(msg) {		
			this.notificationBox.text(msg).show();
			this.msg = $('<span class="icon"></span>').prependTo(this.notificationBox.text(msg));		
			
			notificationHeight = this.notificationBox.height() || notificationHeight;
			
			this.element.trigger('shownotification');
		}
		else {
			this.notificationBox.text('').hide();
			notificationHeight = 0;
			
			this.element.trigger('hidenotification');
		}
		// resize selected-list to fit in the notification
		this.selectedList.height(Math.max(this.element.height()-this.selectedActions.height()-notificationHeight,1));
	},
	...
	_setData: function(key, value) {
		switch (key) {
			...
			case 'sortable':
				// readonly options
				this._messages(
					$.ui.assignment.constants.MESSAGE_WARNING,
					$.ui.assignment.locale.errorReadonly, 
					{option: key}
				);
			default:
				// default behavior
	...
	_prepareLists: function(side, otherSide, opts) {
		...
		list
			.data('assignment.sortable', opts[side].sortable )
			.data('assignment.droppable', opts[side].droppable )
			.data('assignment.draggable', !opts[side].sortable && (opts[otherSide].sortable || opts[otherSide].droppable) );
		...
		if (opts[side].sortable) {
			list.sortable({
				...
				receive: function(event, ui) {
					// DEBUG
					//that._messages(0, "Receive : " + ui.item.data('assignment.optionLink') + ":" + ui.item.parent()[0].className + " = " + itemSelected);
	
					// we received an element from a sortable to another sortable...
					if (opts[otherSide].sortable) {
						var optionLink = ui.item.data('assignment.optionLink');

						that._applyItemState(ui.item.hide(), itemSelected);

						// if the cache already contain an element, remove it
						if (otherList.data('assignment.cache')[optionLink.val()]) {
							delete otherList.data('assignment.cache')[optionLink.val()];
						}
					...
					that._notification($.tmpl($.ui.assignment.locale.itemsDefault, {type:that.options.itemLabel}));
				},
		...
		if (!(opts[side].sortable && opts[otherSide].sortable)
		 && (opts[side].droppable || opts[otherSide].sortable || opts[otherSide].droppable)) {
			...
			list.droppable({
				accept: '.ui-assignment li.ui-element',
				hoverClass: 'ui-state-highlight',
				revert: !(opts[otherSide].sortable || opts[otherSide].droppable),
				greedy: true,
				drop: function(event, ui) {
					// DEBUG
					//that._messages(0, "drop " + side + " = " + ui.draggable.data('assignment.optionLink') + ":" + ui.draggable.parent()[0].className);

					//alert( "drop " + itemSelected );
					// if no optionLink is defined, it was dragged in
					if (!ui.draggable.data('assignment.optionLink')) {
						var optionLink = ui.helper.data('assignment.optionLink');
						ui.draggable.data('assignment.optionLink', optionLink);

						// if the cache already contain an element, remove it
						if (list.data('assignment.cache')[optionLink.val()]) {
							delete list.data('assignment.cache')[optionLink.val()];
						}
						list.data('assignment.cache')[optionLink.val()] = ui.draggable;

						that._applyItemState(ui.draggable, itemSelected);

						that._notification($.tmpl($.ui.assignment.locale.itemsDefault, {type:that.options.itemLabel}));
					// received an item from a sortable to a droppable
					} else if (!opts[side].sortable && !list.is(ui.draggable.parent())) {
						setTimeout(function() { 
							ui.draggable.hide(); 
	...
	_populateLists: function(options) {
		...
		setTimeout(function() {
			$(options.each(function(i) {
				...
				item.data('assignment.idx', i);

				// cache
				list.data('assignment.cache')[item.data('assignment.optionLink').val()] = item;
		...
		var _addNode = function() {
			...
			try {
				...
				if ( _addNodeRetry++ < 10 ) {
					...
					that._messages(
						$.ui.assignment.constants.MESSAGE_EXCEPTION,
						$.ui.assignment.locale.errorInsertNode, 
						{key:node.data('assignment.optionLink').val(), value:node.text()}
					);
					that._setBusy(false);
				}
			}
	...
	_updateCount: function() {
		...
		var count = this.selectedList.children('li:not(.ui-helper-hidden-accessible,.ui-sortable-placeholder)').size();
		...
		this.selectedContainer.find('span.count')
			.text($.tmpl($.ui.assignment.locale.itemsCount, {count:count}))
			.attr('title', $.tmpl($.ui.assignment.locale.itemsTotal, {count:total}));

		if (count === 0) {
			this.selectedContainer.addClass('empty');
		}
		else {
			this.selectedContainer.removeClass('empty');
		}
		var hasDefaultValue = this.selectedDefaultValue() !== '';
		if (hasDefaultValue) {
			this._notification();
		}
		else {
			this._notification($.tmpl($.ui.assignment.locale.itemsDefault, {type:this.options.itemLabel}));
		}
	},
	_getOptionNode: function(option) {
		option = $(option);

		// add radiobutton for items of the selected list
		var radio = option.prop('selected') ? '<input type="radio" name="' + this.options.defaultName + '" value="' + option.val() + '">' : '';
		var node = $('<li class="ui-state-default ui-element"><span class="ui-icon"/>' + radio + option.text() + '<a href="#" class="ui-state-default action"><span class="ui-corner-all ui-icon"/></a></li>').hide();
		if (option.data('default')) {
			node.find(':radio').prop('checked', true);
		}
		node.data('assignment.optionLink', option);
		return node;
	},
	_moveOptionNode: function(item) {
		// call this async to let the item be placed correctly
		setTimeout( function() {
			var optionLink = item.data('assignment.optionLink');
			if (optionLink) {
				var prevItem = item.prev('li:not(.ui-helper-hidden-accessible,.ui-sortable-placeholder):visible');
				var prevOptionLink = prevItem.size() ? prevItem.data('assignment.optionLink') : null;
	...
	_cloneWithData: function(clonee, cacheName, insertItem) {
		var that = this;
		var id = clonee.data('assignment.optionLink').val();
		var selected = ('selected' == cacheName);
		var list = (selected ? this.selectedList : this.availableList);
		var clone = list.data('assignment.cache')[id];

		if (!clone) {
			clone = that._getOptionNode(clonee.data('assignment.optionLink')).hide();
			//clone = clonee.clone().hide();
			this._applyItemState(clone, selected);
			// update cache
			list.data('assignment.cache')[id] = clone;
			// update <option> and idx
			clone.data('assignment.optionLink', clonee.data('assignment.optionLink'));
			// need this here because idx is needed in _getSuccessorNode
			clone.data('assignment.idx', clonee.data('assignment.idx'));
			...
			// uncheck radiobutton
			clone.children(':radio').prop('checked', false);
			list.data('assignment.cache')[id] = clone;
			// update idx
			clone.data('assignment.idx', clonee.data('assignment.idx'));
		}
	...
	_getSuccessorNode: function(item, list) {
		...
		var succ, i = Math.min(item.data('assignment.idx'),itemsSize-1), direction = comparator(item, $(items[i]));
   ...
   // @param bool noclone (optional) true only if item should not be cloned on the other list
	_setSelected: function(item, selected, noclone) {
		...
		if (selected) {
			...
			item.children('input:radio:checked').prop('checked', false);
	...
	_setBusy: function(state) {
		...
		if (state && (1 == this.busy)) {
			if (this.options.searchable) {
				// backup input state
				input.data('assignment.hadFocus', input.data('assignment.hasFocus'));
			...
			if (this.options.searchable) {
				input.show();
				if (input.data('assignment.hadFocus')) input.focus();
			}
			busy.hide();
		}
	...
	_registerHoverEvents: function(elements) {
		elements
			.unbind('mouseover.assignment').bind('mouseover.assignment', function() {
				$(this).find('a').addBack().addClass('ui-state-hover');
			})
			.unbind('mouseout.assignment').bind('mouseout.assignment', function() {
	...
	_registerAddEvents: function(elements) {
		var that = this;
		elements.unbind('click.assignment').bind('click.assignment', function() {
			// ignore if busy...
			if (!this.busy) {
		...
		if (this.availableList.data('assignment.draggable')) {
			// make draggable
			...
			if (this.selectedList.data('assignment.sortable')) {
	...
	_registerRemoveEvents: function(elements) {
		...
		if (this.selectedList.data('assignment.draggable')) {
			// make draggable
			elements.each(function() {
				$(this).parent().draggable({
			...
			// refresh the selected list or the draggable will not connect to it first hand
			if (this.availableList.data('assignment.sortable')) {
	...
	_registerSearchEvents: function(input, searchNow) {
					...
					try {
						$.ajax({
							...
							error: function(request,status,e) {
								that._messages(
									$.ui.assignment.constants.MESSAGE_ERROR, 
									$.ui.assignment.locale.errorRequest, 
									{status:status}
								);
								that._setBusy(false); 
							}
						...
						that._messages($.ui.assignment.constants.MESSAGE_EXCEPTION, e.message);   // error message template ??
						that._setBusy(false); 
					}
				} else {
		...
		input.unbind('focus.assignment blur.assignment keydown.assignment keypress.assignment');
		if (this.options.searchable) {
			input
			.bind('focus.assignment', function() {
				$(this).addClass('ui-state-active').data('assignment.hasFocus', true);
			})
			.bind('blur.assignment', function() {
				$(this).removeClass('ui-state-active').data('assignment.hasFocus', false);
			})
			.bind('keydown.assignment keypress.assignment', function(e) {
...
var _dragHelper = function(event, ui) {
	...
	clone
		.data('assignment.optionLink', item.data('assignment.optionLink'))
		.data('assignment.list', item.parent() )
		// node ui cleanup
		.find('a').remove()
	;
	return clone;
...
$.extend($.ui.assignment, {
	getter: 'selectedValues selectedDefaultValue isSelected enabled isBusy widget resize',
	locale: {
		addAll:'Add all',
		...
		itemsDefault: 'Please define a default #{type}.',
		busy:'please wait...',
