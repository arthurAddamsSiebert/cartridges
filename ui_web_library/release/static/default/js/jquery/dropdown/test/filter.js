(function($){
  ...
  function getVisible() {
    return widget.find(".ui-dropdown-checkboxes input:visible");
  }

  function getChecked() {
    return el.dropdown("getChecked");
  }
  ...
  module("filter widget - multiple select", {
    setup: function() {
      ...
      input = widget.find(".ui-dropdown-filter input");
      button = el.next();
    },

    teardown: function() {
      el.dropdownfilter("destroy");
      el.dropdown("destroy");
      el.remove();
    }
  });
