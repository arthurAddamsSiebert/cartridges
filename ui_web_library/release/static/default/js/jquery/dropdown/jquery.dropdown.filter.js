    ...
    $.widget("ech.dropdownfilter", {
        ...
        _create: function(){
            var self = this,
                ...
                header = (this.header = instance.menu.find(".ui-dropdown-header").addClass("ui-dropdown-hasfilter")),

                // wrapper elem
                wrapper = (this.wrapper = $('<div class="ui-dropdown-filter"><input placeholder="'+(opts.label.length ? opts.label : '')+'" type="text"' + (/\d/.test(opts.width) ? 'style="width:'+opts.width+'px"' : '') + ' /></div>').prependTo( this.header ));
            ...
            this.element.on('dropdownopen', function() {
                setTimeout(function(){
                    self.input.focus();
                }, 1);
            });
