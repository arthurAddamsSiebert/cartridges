...
public final class ChannelCategoryAssignmentValidationRuleConfiguration_jsp extends com.intershop.beehive.core.internal.template.AbstractTemplate
...
response.setHeader(TemplateConstants.PERSONALIZED_HEADER, "1");setEncodingType("application/json"); 
 ...
 while (loop("ClassificationCatalogUUIDs","uuid",null)) { 
      out.write('"');
 {String value = null;try{value=context.getFormattedValue(getObject("uuid"),null,null);}catch(Exception e){value=null;Logger.error(this,"ISPRINT has an invalid expression. Returning empty string. Line: {34}",e);}if (value==null) value="";value = encodeString(value);out.write(value);} 
      out.write('"');
 _boolean_result=false;try {_boolean_result=((Boolean)((hasNext("uuid") ? Boolean.TRUE : Boolean.FALSE))).booleanValue();} catch (Exception e) {Logger.debug(this,"Boolean expression in line {} could not be evaluated. False returned. Consider using the 'isDefined' ISML function.",34,e);}if (_boolean_result) { 
      out.write(',');
 } 
 } 
      out.write(" \n]\n}");
