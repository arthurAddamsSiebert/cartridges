CREATE OR REPLACE PROCEDURE sp_deleteUserDelegateByDomain (domainID IN VARCHAR2)

------------------------------------------------------------------------------------------
-- Name        : deleteUserDelegationByDomain
...
AS
    ...
    BEGIN
       ...
       EXECUTE IMMEDIATE 'DELETE FROM '||staging.get_table_name('UserDelegation', suffix)||' WHERE domainid = :domainid' USING in_domainid;
       COMMIT;
...
BEGIN
    ...
    IF sf_is_staging_table('UserDelegation') THEN
        --
...
END sp_deleteUserDelegateByDomain;
/
