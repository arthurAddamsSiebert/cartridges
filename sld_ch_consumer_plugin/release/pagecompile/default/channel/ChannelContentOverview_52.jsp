...
new TagParameter("text",localizeText(context.getFormattedValue("ChannelContentOverview_52.Content.text",null)))}, 7); %><% {try{executePipeline("ProcessContentRepository-GetContentRepository",java.util.Collections.emptyMap(),"cms");}catch(Exception e){Logger.error(this,"ISPIPELINE failed. Line: 9.",e);}} %><div class="overviewWrapper">
<div class="overview_title"><% processOpenTag(response, pageContext, "apptext", new TagParameter[] {
new TagParameter("key","content.title")}, 11); %></div>
...
new TagParameter("key","content.description")}, 12); %><br/>&nbsp;</div><% {out.flush();processExtensionPoint((com.intershop.beehive.core.capi.request.ServletResponse)response,"ChannelContentOverviewEntries", null, "13");} %></div><% printFooter(out); %>