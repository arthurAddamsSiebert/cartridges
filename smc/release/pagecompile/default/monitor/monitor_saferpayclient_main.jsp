...
<tr>
<td class="table_title aldi"><% {out.write(localizeISText("monitor_saferpayclient_main.SaferpayClientInstallationOverview.table_title","",null,null,null,null,null,null,null,null,null,null,null));} %></td>
</tr>
<tr>
<td class="table_title_description w e"><% {out.write(localizeISText("monitor_saferpayclient_main.ThisPageProvidesAnOverviewAboutSaferpay.table_title_description",null,null,url(true,(new URLPipelineAction(context.getFormattedValue("PerformLogout-GlobalLogout",null)))),"table_title_description",null,null,null,null,null,null,null,null));} %></td>
</tr>
<tr>
<td>
...
<td class="table_header s e" nowrap="nowrap"><% {out.write(localizeISText("monitor_saferpayclient_main.Installation.table_header","",null,null,null,null,null,null,null,null,null,null,null));} %></td>
<td class="table_header s e" nowrap="nowrap"><% {out.write(localizeISText("monitor_saferpayclient_main.Communication.table_header","",null,null,null,null,null,null,null,null,null,null,null));} %></td>
<td class="table_header s e" nowrap="nowrap"><% {out.write(localizeISText("monitor_saferpayclient_main.BusinessInterface.table_header","",null,null,null,null,null,null,null,null,null,null,null));} %></td> 
</tr> 
<% while (loop("ServerRegistrations","TargetServer",null)) { %><% _boolean_result=false;try {_boolean_result=((Boolean)((((context.getFormattedValue(getObject("TargetServer:Type"),null).equals(context.getFormattedValue("AppServer",null)))) ? Boolean.TRUE : Boolean.FALSE))).booleanValue();} catch (Exception e) {Logger.debug(this,"Boolean expression in line {} could not be evaluated. False returned. Consider using the 'isDefined' ISML function.",23,e);}if (_boolean_result) { %><% {out.flush();%><%@page import="com.intershop.beehive.core.capi.url.*"%><%URLRewriteHandler handler = getTemplateExecutionConfig().getURLRewriteHandler();
try
{
...
<td class="table_detail e" valign="top" align="right"><% {String value = null;try{value=context.getFormattedValue(getObject("TargetServer:Port"),"#",null);}catch(Exception e){value=null;Logger.error(this,"ISPRINT has an invalid expression. Returning empty string. Line: {29}",e);}if (value==null) value="";value = encodeString(value);out.write(value);} %>&nbsp;</td>
